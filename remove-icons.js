...
const iconPatterns = [
  // Remove icon imports
  { pattern: /import\s*{[^}]*}\s*from\s*"lucide-react";/g, replacement: 'import {} from "lucide-react";' },
  
  // Remove icon elements with className
  { pattern: /<[A-Z][a-zA-Z]*\s+className="[^"]*"\s*\/>/g, replacement: '' },
  
  // Remove icon elements with other props
  { pattern: /<[A-Z][a-zA-Z]*\s+[^>]*className="[^"]*"[^>]*\/>/g, replacement: '' },
  
  // Replace specific icon usages in JSX
  { pattern: /<ArrowLeft className="h-4 w-4"\s*\/>/g, replacement: '←' },
  { pattern: /<Loader2 className="[^"]*animate-spin[^"]*"\s*\/>/g, replacement: '' },
  
  // Remove icon containers
  { pattern: /<div className="p-3 rounded-xl bg-[^"]*border[^"]*">\s*<[A-Z][a-zA-Z]*[^>]*\/>\s*<\/div>/g, replacement: '' },
];
